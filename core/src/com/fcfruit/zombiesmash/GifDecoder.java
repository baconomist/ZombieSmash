...
public class GifDecoder {
    ...
    protected void decodeBitmapData() {
        ...
        for (i = 0; i < npix;) {
            if (top == 0) {
                if (bits < code_size) {
                    // Load bytes until there are enough bits for EnvControl code.
                    if (count == 0) {
                        // Read EnvControl new data block.
                        count = readBlock();
                        if (count <= 0) {
                            break;
                        }
                ...
                // Add EnvControl new string to the string table,
            ...
            // Pop EnvControl pixel off the pixel stack.
            top--;
    ...
    /**
     * Reads EnvControl single byte from the input stream.
     */
    protected int read() {
        int curByte = 0;
        try {
